...
import { useTranslation } from "react-i18next";
import SEO from "../components/SEO";
import { useMissionVisionPage } from "../api/strapi";

export default function MssionVision() {
  const { t, i18n } = useTranslation();
  const isRTL = i18n.dir() === "rtl";

  const { data } = useMissionVisionPage();

  const sections = [
    {
      imgUrl:
        data?.missionImage ||
        "/images/MssionVsion/WhatsApp Image 2018-11-05 at 11.31.24.jpeg",

      h1: data?.missionTitle || t("missionVisionPage.sections.mission.title"),

      p:
        data?.missionDescription ||
        t("missionVisionPage.sections.mission.description"),
    },

    {
      ...
      h1: data?.visionTitle || t("missionVisionPage.sections.vision.title"),

      p:
        data?.visionDescription ||
        t("missionVisionPage.sections.vision.description"),
    },
  ];
  return (
    <div>
      <SEO
        ...
        description={
          i18n.language?.startsWith("ar")
            ? "تعرف على رؤية ورسالة مستشفى القصيم الوطني."
            : "Learn about the mission and vision of Qassim National Hospital."
        }
      />
        ...
        {sections.map((section, index) => (
          ...
          >
            <img
              src={section.imgUrl}
              alt={section.h1}
              className="w-full max-w-[400px] flex-shrink-0 rounded-2xl object-cover shadow-md md:h-[350px] md:w-[400px]"
            />
